...
export function useDeployProject() {
    ...
    return useMutation({
        mutationFn: async ({ id, networkConfig }: DeployProjectParams) => {
            const { data } = await apiClient.post<{
                message: string;
                project: Project;
            }>(`/projects/${id}/deploy`, { networkConfig });
            return data.project;
        },
        onSuccess: (project) => {
            queryClient.invalidateQueries({ queryKey: ["projects"] });
...
export function useVerifyProject() {
    ...
    return useMutation({
        mutationFn: async (id: string) => {
            const { data } = await apiClient.post<{ message: string }>(`/projects/${id}/verify`);
            return data;
        },
        onSuccess: (_data, id) => {
            queryClient.invalidateQueries({ queryKey: ["projects"] });
