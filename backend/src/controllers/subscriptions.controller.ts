...
import { verifyPaymentTransaction, getPlanPrice } from "../services/payment-verification";

export async function getSubscription(req: AuthRequest, res: Response) {
    try {
        const walletAddress = req.user!.walletAddress;

        const user = await await User.findOne({ $or: [{ walletAddress }, { privyUserId: req.user!.privyUserId }] });
        if (!user) {
...
export async function createPaymentIntent(req: AuthRequest, res: Response) {
    try {
        ...
        const user = await User.findOne({ $or: [{ walletAddress }, { privyUserId: req.user!.privyUserId }] });
        if (!user) {
            return res.status(404).json({ error: "User not found" });
        }
        ...
        res.json({
            ...
            recipientAddress: process.env.PAYMENT_RECIPIENT_ADDRESS || "0x0000000000000000000000000000000000000000",
        });
    } catch (error: any) {
...
export async function verifyPayment(req: AuthRequest, res: Response) {
    try {
        ...
        const expectedAmount =
            subscription.paymentAmount || (subscription.plan !== "free" ? getPlanPrice(subscription.plan) : "0.00");
        const verification = await verifyPaymentTransaction(txHash, expectedAmount, walletAddress);
        ...
        if (verification.confirmed) {
            ...
            log.success(`Subscription activated for user ${walletAddress}: ${subscription.plan} (tx: ${txHash})`);
        } else {
            log.warn(`Payment verification failed for subscription ${subscriptionId} (tx: ${txHash})`);
        }
